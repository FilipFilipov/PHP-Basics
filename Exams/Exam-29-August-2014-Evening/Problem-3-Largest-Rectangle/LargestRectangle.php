...
function isRectangle($input, $minRow, $minCol, $maxRow, $maxCol) {
    $targetChar = $input[$minRow][$minCol];
    for ($col = $minCol; $col <= $maxCol; $col++) {
        if ($input[$minRow][$col] !== $targetChar) {
            return false;
        }
        if ($input[$maxRow][$col] !== $targetChar) {
            return false;
        }
    }
    for ($row = $minRow; $row <= $maxRow; $row++) {
        if ($input[$row][$minCol] !== $targetChar) {
            return false;
        }
        if ($input[$row][$maxCol] !== $targetChar) {
            return false;
        }
    }
    return true;
